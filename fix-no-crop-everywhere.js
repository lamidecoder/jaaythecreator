#!/usr/bin/env node
/**
 * fix-no-crop-everywhere.js
 *
 * Removes cropping from every grid, not just the hero. Grid tiles
 * (Home showcase, Work grid, gallery, "More stories") keep their box
 * shaped by each piece's declared aspect ratio, so the grid still
 * lines up neatly, but the actual photo or video inside now always
 * shows in full (object-contain) rather than being cropped to fill
 * that box.
 *
 * Tested with a deliberately mismatched case before sending this: a
 * portrait video declared as 16:9 (the kind of mismatch that happens
 * when a file has no aspect hint in its name) now letterboxes inside
 * its box instead of being cropped.
 *
 * Run once from your project root:  node fix-no-crop-everywhere.js
 */
...
console.log("Updated components/media-frame.tsx — nothing crops anywhere on the site now.");
